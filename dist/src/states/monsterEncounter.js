...
function resolveCombatRound(player, event) {
    ...
    if (monster.hp <= 0) {
        ...
        return result([
            gameMessage(`${monsterDefeatedText(combatMessage, monster)} ${monsterRewardText(reward)} ${location.desc}`, getDisplayActions(location.actions)),
        ], { shouldSave: true });
    }
    return result([
        gameMessage(combatStatusText(combatMessage, player, monster), getCommandLabels(EVENT_ACTIONS.monster)),
    ], { shouldSave: true });
}
